...
const Navigation = () => {
  ...
  const [cartItems, setCartItems] = useState<CartItem[]>([
    { id: 1, name: "Pikachu", price: "€120", image: pikachuImage, quantity: 1, category: "Electric" },
    { id: 2, name: "Charizard", price: "€350", image: charizardImage, quantity: 1, category: "Fire" },
    { id: 3, name: "Bulbasaur", price: "€95", image: bulbasaurImage, quantity: 1, category: "Grass" }
  ]);
  ...
  useEffect(() => {
    ["/rings-collection.png", "/earrings-collection.png", "/arcus-bracelet.png", "/span-bracelet.png", "/founders.png"].forEach(src => {
      const img = new Image();
      img.src = src;
    });
  }, []);

  const popularSearches = ["Pikachu", "Charizard", "Eevee", "Legendary Pokémon", "Starter Pokémon", "Shiny Pokémon"];
  
  const navItems = [
    { 
      name: "Shop", 
      href: "/category/shop",
      submenuItems: ["Fire Type", "Water Type", "Grass Type", "Electric Type", "Psychic Type"],
      images: [
        { src: "/rings-collection.png", alt: "Electric Type Pokémon", label: "Electric Type" },
        { src: "/earrings-collection.png", alt: "Water Type Pokémon", label: "Water Type" }
      ]
    },
    { 
      name: "New in", 
      href: "/category/new-in",
      submenuItems: ["This Week's Catches", "Legendary Drops", "Shiny Collection", "Limited Edition", "Pre-Orders"],
      images: [
        { src: "/arcus-bracelet.png", alt: "Pikachu", label: "Pikachu" },
        { src: "/span-bracelet.png", alt: "Charizard", label: "Charizard" }
      ]
    },
    { 
      name: "About", 
      href: "/about/our-story",
      submenuItems: ["Our Story", "Pokémon Care", "Trainer Guide", "Customer Care", "Store Locator"],
      images: [
        { src: "/founders.png", alt: "Our Team", label: "Read our story" }
      ]
    }
  ];
  ...
  return (
    <nav className="relative" style={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', backdropFilter: 'blur(10px)' }}>
      <div className="flex items-center justify-between h-16 px-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button className="p-2 text-nav-foreground hover:text-nav-hover transition-colors duration-200 relative" aria-label="Shopping bag" onClick={() => setIsShoppingBagOpen(true)}>
            ...
            {totalItems > 0 && <span className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-[30%] text-[0.5rem] font-semibold text-black pointer-events-none">{totalItems}</span>}
          </button>
        </div>
      </div>
              ...
              <div className="flex-1">
                <ul className="space-y-2">
                  {navItems.find(item => item.name === activeDropdown)?.submenuItems.map((subItem, index) => (
                    <li key={index}>
                      <Link to={activeDropdown === "About" ? `/about/${subItem.toLowerCase().replace(/\s+/g, '-')}` : `/category/${subItem.toLowerCase().replace(/\s+/g, '-')}`} className="text-nav-foreground hover:text-nav-hover transition-colors duration-200 text-sm font-light block py-2">{subItem}</Link>
                    </li>
                  ))}
                </ul>
              </div>
              <div className="flex space-x-6">
                {navItems.find(item => item.name === activeDropdown)?.images.map((image, index) => {
                  ...
                  if (activeDropdown === "Shop") {
                    if (image.label === "Electric Type") linkTo = "/category/electric-type";
                    else if (image.label === "Water Type") linkTo = "/category/water-type";
                  } else if (activeDropdown === "New in") {
                    if (image.label === "Pikachu") linkTo = "/product/1";
                    else if (image.label === "Charizard") linkTo = "/product/2";
            ...
            <div className="max-w-2xl mx-auto">
              <div className="relative mb-8">
                <div className="flex items-center border-b border-border pb-2">
                  ...
                  <input type="text" placeholder="Search for Pokémon..." className="flex-1 bg-transparent text-nav-foreground placeholder:text-nav-foreground/60 outline-none text-lg" autoFocus />
                </div>
              </div>
              <div>
                ...
                <div key={item.name}>
                  ...
                  <div className="mt-3 pl-4 space-y-2">
                    {item.submenuItems.map((subItem, subIndex) => (
                      <Link key={subIndex} to={item.name === "About" ? `/about/${subItem.toLowerCase().replace(/\s+/g, '-')}` : `/category/${subItem.toLowerCase().replace(/\s+/g, '-')}`} className="text-nav-foreground/70 hover:text-nav-hover text-sm font-light block py-1" onClick={() => setIsMobileMenuOpen(false)}>{subItem}</Link>
                    ))}
                  </div>
                </div>
              ))}
      ...
      <ShoppingBag isOpen={isShoppingBagOpen} onClose={() => setIsShoppingBagOpen(false)} cartItems={cartItems} updateQuantity={updateQuantity} onViewFavorites={() => { setIsShoppingBagOpen(false); setOffCanvasType('favorites'); }} />
      ...
      {offCanvasType === 'favorites' && (
        <div className="fixed inset-0 z-50 h-screen">
          ...
          <div className="absolute right-0 top-0 h-screen w-96 bg-background border-l border-border animate-slide-in-right flex flex-col">
            ...
            <div className="p-6">
              <p className="text-muted-foreground text-sm mb-6">You haven't added any favorites yet. Browse our Pokémon collection and click the heart icon to save the ones you love.</p>
            </div>
          </div>
        </div>
      )}
